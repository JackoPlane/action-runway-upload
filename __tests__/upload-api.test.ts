...
const { RunwayUploadApi, RUNWAY_UPLOAD_API_HOSTNAME } = await import(
  '../src/runway/upload-api.js'
)
...
describe('upload-api.ts', () => {
  ...
  describe('uploadAdditionalFileToBuild', () => {
    ...
    it('handles default filename when fileName is not provided and filePath has no directory', async () => {
      ...
      expect(formDataMock.append).toHaveBeenCalledWith(
        'data',
        JSON.stringify({ fileName: 'simple-file.txt' })
      )
