...
jest.unstable_mockModule('../src/utils/git.js', () => ({
  getLastGitCommit: jest.fn<GetLastGitCommitType>().mockResolvedValue(mockLastGitCommit)
}))

// Import the mocked git helper so we can manipulate it in tests
const gitModule = await import('../src/utils/git.js')
const getLastGitCommit = gitModule.getLastGitCommit as jest.MockedFunction<GetLastGitCommitType>
...
describe('build-metadata.ts', () => {
  ...
  it('returns complete metadata when all GitHub context is available', async () => {
    ...
    if (metadata.ciBuildInfo) {
      ...
      expect(metadata.ciBuildInfo.url).toBe('https://github.com/test-owner/test-repo/actions/runs/12345')
      
      // Verify commit info
      expect(metadata.ciBuildInfo.commitHash).toBe('abcdef1234567890abcdef1234567890abcdef12')
      ...
      expect(metadata.ciBuildInfo.commitUrl).toBe('https://github.com/test-owner/test-repo/commit/abcdef1234567890abcdef1234567890abcdef12')
      ...
      if (metadata.ciBuildInfo.workflowData) {
        ...
        expect(metadata.ciBuildInfo.workflowData.workflowName).toBe('Test Workflow')
      }
    }
  })
